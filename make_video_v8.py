...
粉粉成长视频生成器 - v8 最终修复版
✅ 所有年龄段照片（201张）
✅ 正确的背景音乐（使用Python生成WAV+FFmpeg转MP3）
✅ 自适应照片尺寸
✅ 正确显示日期和文案
"""
...
# ==================== 生成背景音乐 ====================
def generate_music(duration=120, output_file=None):
    """使用Python生成WAV音乐，然后转换为MP3"""
    print(f"🎵 生成背景音乐 (时长: {duration}秒)...")
    
    # 生成WAV
    wav_file = output_file.replace('.mp3', '.wav') if output_file else None
    ...
    for i in range(num_samples):
        # 主频率 + 和声
        sample = (
            0.3 * math.sin(2 * math.pi * 262 * i / sample_rate) +  # C4
            0.15 * math.sin(2 * math.pi * 330 * i / sample_rate) +  # E4
            0.1 * math.sin(2 * math.pi * 392 * i / sample_rate)     # G4
        )
        # 限制音量范围
        sample = max(-1, min(1, sample))
        sample_int = int(32767 * sample)
    ...
    print(f"🎵 WAV生成完成: {os.path.getsize(wav_file)} bytes")
    ...
    if os.path.exists(output_file):
        print(f"🎵 MP3生成完成: {os.path.getsize(output_file)} bytes")
        return True
    else:
        print(f"🎵 MP3生成失败: {result.stderr[-200:]}")
        return False
...
if generate_music(duration=120, output_file=audio_file):
    print("🎵 背景音乐生成成功！")
else:
    print("🎵 背景音乐生成失败，将生成无音频版本")
