...
with open(list_file, "w", encoding="utf-8") as f:
    ...
    for fn in frame_files:
        f.write(f"file 'p{len(frame_files)-1:04d}.jpg'\nduration {SHOW_SEC}\n")
    # 修正：使用正确的文件名
list_file2 = os.path.join(TEMP, "list.txt")
...
print("📝 视频列表内容:")
with open(list_file2, "r", encoding="utf-8") as f:
    lines = f.readlines()
...
if os.path.exists(output):
    if os.path.exists(audio_file):
        ...
        final = os.path.join(TEMP, "video_with_audio.mp4")
        if os.path.exists(final):
            shutil.copy2(final, OUTPUT_FILE)
        else:
